...
// ListGitTemplateFiles reads the content of a specific labels from static/bindata or custom path.
func ListGitTemplateFiles(name string) ([]string, error) {
	return AssetFS().ListFiles("template/"+name, true)
}

// GitTemplate reads the content of a specific labels from static/bindata or custom path.
func GitTemplate(templatePath, name string) ([]byte, error) {
	return AssetFS().ReadFile("template", templatePath, name)
}

// GetGitTemplateBaseDir get the dir of a specific labels from static/bindata or custom path.
func GetGitTemplateBaseDir(templatePath string) string {
	return AssetFS().GetBaseDir("template", templatePath)
}
