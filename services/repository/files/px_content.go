...
func isNonText(entry *git.TreeEntry) (bool, error) {
	...
	if err != nil {
		log.Error("error happened in reading text : %v", err)
		return false, fmt.Errorf("failed to read from data source: %w", err)
	}
	buf = buf[:n]
	...
	size := 0
	nums := 0
	for start := 0; start < len(buf) && nums < 10; start += size {
		var r rune
