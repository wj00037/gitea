...
// ChangeRepoFilesWithGitAtt adds, updates or removes multiple files in the given repository
func ChangeRepoFilesWithGitAtt(ctx context.Context, repo *repo_model.Repository, doer *user_model.User, opts *ChangeRepoFilesOptions) (*structs.FilesResponse, error) {
	...
	ta, err2 := NewTemporaryCheckAttributeRepository(ctx, repo)
	if err2 != nil {
		log.Error("%v", err)
	}
	...
	if err := ta.FullyClone(opts.OldBranch); err != nil {
		return nil, fmt.Errorf("fail to fully clone repo: %w", err)
	}
	...
	for _, file := range opts.Files {
		...
		case "create", "update":
			if err := createOrUpdateFileWithGitAtt(t, ta, file, opts.OldBranch, hasOldBranch); err != nil {
				return nil, err
			}
		case "delete":
...
// createOrUpdateFileWithGitAtt handles creating or updating a file for ChangeRepoFilesWithGitAtt
func createOrUpdateFileWithGitAtt(t *TemporaryUploadRepository, t2 *TemporaryUploadRepository, file *ChangeRepoFile, OldBranch string, hasOldBranch bool) error {
	...
	var attributeFileObjectHash string
	var buf bytes.Buffer

	if setting.LFS.StartServer && hasOldBranch {
		// Check there is no way this can return multiple infos
		filename2attribute2info, err := t2.gitRepo.CheckAttribute(git.CheckAttributeOpts{
		...
		if err == nil && pointer.IsValid() && filename2attribute2info[file.Options.treePath] != nil && filename2attribute2info[file.Options.treePath]["filter"] != "lfs" {
			...
			if content != "" {
				attributeFileObjectHash, err = t.HashBase64Object(content)
				if err := t.AddObjectToIndex("100644", attributeFileObjectHash, ".gitattributes"); err != nil {
					return err
				}
			}
			if err != nil {
	...
	// Add the object to the database
	objectHash, err := t.HashObject(&buf)
	if err != nil {
		return err
	}
